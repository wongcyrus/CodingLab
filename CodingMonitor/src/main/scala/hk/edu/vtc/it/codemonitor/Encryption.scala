...
object Encryption {
  ...
  def decrypt(key: String, encryptedValue: String): Option[String] = {
    val cipher: Cipher = Cipher.getInstance("AES/ECB/PKCS5PADDING")
    cipher.init(Cipher.DECRYPT_MODE, keyToSpec(key))
    try {
      Option(new String(cipher.doFinal(Base64.decodeBase64(encryptedValue))))
    }
    catch {
  ...
  private val SALT: String =
    "jMhKlOuJnM34G6NHkqo9V010GhLAqOpF0BePojHgh1HgNg8^123"
}